...
@Entity
@Table(name = "radiology_exam_request_short")
@SequenceGenerator(name = "ID_SEQ_GEN", initialValue = 1, sequenceName = "RAD_EXAM_REQUEST_SHORT_SEQ", allocationSize = 1)
public class RadiologyExamRequestShort extends RadiologyExamRequestBase {

    private static final long serialVersionUID = 7941120101543587862L;

    @OneToMany(mappedBy = "radiologyExamRequest", cascade = CascadeType.REFRESH, fetch = FetchType.LAZY)
    private List<RadiologyExamRequestItemShort> radiologyExamRequestItems;

    @Column(name = "CDA2_SIGN_DATE")
    private Date cda2SignDate;

    @OneToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "cda2_file_entity_id", foreignKey = @ForeignKey(name = "FK_RAD_EX_REQ_SHRT_CDA2_FILE_ENTITY"))
    private FileEntity cda2FileEntity;

    @Override
    public List<RequestNote> getRequestNotes() {
        try {
            return DaoManager.load(RequestNote.class, new Criterion[]{
                    Restrictions.eq("radiologyExamRequest.id", getId())
            });
    ...
    @Transient
    private List<RadiologyExamRequestItemShort> selectedRadItemsList;

    @Transient
    private List<RadiologyExamRequestItemShort> itemsForTag;
    ...
    public String getExamsDescription() throws HibernateException,
        ...
        if (!ValidationHelper.isNullOrEmpty(getRadiologyExamRequestItems())) {
            List<RadiologyExamRequestItemShort> items = null;
            if (!ValidationHelper.isNullOrEmpty(getRadiologyExamRequestItems().get(0))
                    && !ValidationHelper.isNullOrEmpty(getRadiologyExamRequestItems().get(0).getFileEntity())) {
                FileEntity fileEntity = getRadiologyExamRequestItems().get(0).getFileEntity();
                items = DaoManager.load(RadiologyExamRequestItemShort.class, new Criterion[]{
    ...
    public String getItemExecutionDateForTag() {
        if (!ValidationHelper.isNullOrEmpty(getAllItemsForTag())) {
            ...
            List<String> executionDate = new ArrayList<>();

            for (RadiologyExamRequestItemShort item : getItemsForTag()) {
    ...
    public void udateItemsDescription(List<RadiologyExamRequestItemShort> items)//FIXME: DELETE AFTER CREATE TRIGGER
    {
        if (!ValidationHelper.isNullOrEmpty(items)) {
            StringBuffer sb = new StringBuffer();

            for (RadiologyExamRequestItemShort item : items) {
                sb.append(", ");
    ...
    public String getItemTsrmForTag() {
        if (!ValidationHelper.isNullOrEmpty(getAllItemsForTag())) {
            List<String> trsm = new ArrayList<>();
    ...
    public String getCardNumberForTag() {
        if (!ValidationHelper.isNullOrEmpty(getAllItemsForTag())) {
            StringBuffer sb = new StringBuffer();
            List<Long> cardNumber = new ArrayList<>();
            List<Long> cardYear = new ArrayList<>();
            ...
            for (int i = 0; i < cardNumber.size(); ++i) {
                if (cardNumber.get(i) != null) {
                    ...
                    sb.append(getExamType().getDescription(), 0, 2);
    ...
    private List<RadiologyExamRequestItemShort> getAllItemsForTag() {
        ...
        if (ValidationHelper.isNullOrEmpty(ids)) {
            if (!ValidationHelper.isNullOrEmpty(getRadiologyExamRequestItems())
                    && !ValidationHelper.isNullOrEmpty(getRadiologyExamRequestItems().get(0).getFileEntity())) {
                try {
                    List<RadiologyExamRequestItemShort> radItemsWithSameFileEntity = DaoManager
                            .load(RadiologyExamRequestItemShort.class, new Criterion[]{
                                    Restrictions.eq("fileEntityId",
                                            getRadiologyExamRequestItems().get(0).getFileEntity().getId())
                            });
            ...
            try {
                setItemsForTag(DaoManager.load(RadiologyExamRequestItemShort.class, new Criterion[]{
    ...
    public RadExamRequestWrapper getRadExamRequestWrapperFromRequest() {
        if (!ValidationHelper.isNullOrEmpty(this.getRadiologyExamRequestItems())) {
            RadExamRequestWrapper requestWrapper = new RadExamRequestWrapper(this.getId(), this.getExamType(),
                    this.getLatestActionDate(), null, this.getPatient(),
                    this.getRadiologyExamRequestItems().get(0).getAccessNumber(),
            ...
            List<RadExamRequestItemWrapper> radExamRequestItemWrappers = new ArrayList<>();
            for (RadiologyExamRequestItemShort reri : this.getRadiologyExamRequestItems()) {
                radExamRequestItemWrappers.add(new RadExamRequestItemWrapper(
                        reri.getId(), reri.getRadiologyExam(), reri.getRequestingDoctor(), reri.getReserveDate(),
                        reri.getRequestDate(), this.getId(), reri.getDiagnosticQuestion(), reri.getAccessNumber(),
                        requestWrapper, reri.getAddedManually()));
            }
    ...
    @Override
    public List<RadiologyExamRequestItemShort> getRadiologyExamRequestItems() {
        return radiologyExamRequestItems;
    }

    public void setRadiologyExamRequestItems(List<RadiologyExamRequestItemShort> radiologyExamRequestItems) {
    ...
    public String getRadiologyExamRequestItemsString() {
        ...
        if (this.getRadiologyExamRequestItems() != null) {
            for (RadiologyExamRequestItemShort item : this.getRadiologyExamRequestItems()) {
                sb.append(item.getRadiologyExam().getDescription());
                sb.append(", ");
            }
            if (sb.length() > 2) {
    ...
    public boolean getSelectedAll() {
        if (this.getRadiologyExamRequestItems() != null) {
            for (RadiologyExamRequestItemShort radiologyExamRequestItem : this.getRadiologyExamRequestItems()) {
    ...
    public List<RadiologyExamRequestItemShort> getSelectedRadItemsList() {
        return selectedRadItemsList;
    }

    public void setSelectedRadItemsList(List<RadiologyExamRequestItemShort> selectedRadItemsList) {
    ...
    public List<RadiologyExamRequestItemShort> getItemsForTag() {
        return itemsForTag;
    }

    public void setItemsForTag(List<RadiologyExamRequestItemShort> itemsForTag) {
        this.itemsForTag = itemsForTag;
    }

    @Override
    public RadiologyExamRequestShort clone() throws CloneNotSupportedException {
        RadiologyExamRequestShort obj = new RadiologyExamRequestShort();
